...
describe('TelemetryService', () => {
  it('accepts a duplicate without queueing it twice', async () => {
    const prisma = {
      session: { findUnique: vi.fn().mockResolvedValue({ id: 's1' }) },
      telemetryEvent: { create: vi.fn().mockRejectedValue(new Prisma.PrismaClientKnownRequestError('duplicate', { code: 'P2002', clientVersion: 'test' })) },
    } as any;
